...
class Tower:
    ...
    def draw_range(self, surface):
        ...
        pygame.draw.circle(range_surf, (255, 255, 255, 60), (self.range, self.range), self.range)
        pygame.draw.circle(range_surf, WHITE, (self.range, self.range), self.range, 1)
...
class Map:
    ...
    def _calculate_invalid_cells(self):
        ...
        Az út vastagsága miatt a környező cellákat is blokkolja.
        """
        path_width = 40 # Az út szélessége
        ...
        for i in range(len(self.path) - 1):
            ...
            # Végig lépkedünk az útszakaszon pixelről pixelre
            for step in range(steps + 1):
                t = step / steps if steps > 0 else 0
                current_point = p1.lerp(p2, t)
                
                # Meghatározzuk az adott pont rács koordinátáját
                gx = int(current_point.x) // GRID_SIZE
                gy = int(current_point.y) // GRID_SIZE
                
                # Blokkoljuk a cellát és a közvetlen szomszédait (az út vastagsága miatt)
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        self.invalid_grid_cells.add((gx + dx, gy + dy))
...
class Game:
    ...
    def __init__(self):
        ...
        # 3 nagy főgomb
        big_btn_w, big_btn_h = 260, 90
        cx = SCREEN_WIDTH // 2 - big_btn_w // 2
        self.main_menu_buttons = [
            Button(cx, 280, big_btn_w, big_btn_h, "Szintek", self.font_medium, DARK_GRAY, WHITE, lambda: self._open_panel("levels")),
            Button(cx, 400, big_btn_w, big_btn_h, "Inventory", self.font_medium, DARK_GRAY, WHITE, lambda: self._open_panel("inventory")),
        ...
        self.inventory_back_button = Button(60, 700, 180, 45, "← Vissza", self.font_small, DARK_GRAY, WHITE, lambda: self._open_panel(None))
        self.inv_open_crate_btn = Button(60, 540, 180, 45, "Láda nyitás", self.font_small, (255, 140, 0), BLACK, self.open_crate)
        self.inv_buy_key_btn = Button(250, 540, 180, 45, "Kulcs (100g)", self.font_small, GOLD, BLACK, self.buy_key)
        self.inv_expand_btn = Button(60, 595, 370, 40, f"Bővítés (+{INVENTORY_EXPAND_AMOUNT} hely) — {INVENTORY_EXPAND_COST} Gold", self.font_small, DARK_GRAY, GOLD, self.expand_inventory)
    ...
    def get_equipped_bonuses(self):
        ...
        for item_name in self.equipped_items:
            if item_name and item_name in ITEMS:
                _, rarity, d, s, r, _ = (item_name,) + ITEMS[item_name]
                dmg += d
                slow += s
                rng += r
        return dmg, slow, rng
    ...
    def sell_item(self, item_name):
        ...
        self.floating_texts.append(
            FloatingText(SCREEN_WIDTH//2 - 80, 200, f"+{price} Gold", GOLD, self.font_medium, duration=120)
        )

    def expand_inventory(self):
        """Kibővíti az inventory méretet +5 slottal, ha van elég gold."""
        if self.gold >= INVENTORY_EXPAND_COST:
            self.gold -= INVENTORY_EXPAND_COST
    ...
    def reset_game_data(self):
        ...
        self.hud_buttons = [           
            ...
            Button(SCREEN_WIDTH - btn_w - spacing, hud_y, btn_w, btn_h, "Köv. Hullám", self.font_medium, GREEN, BLACK, self.trigger_next_wave),
            ]
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if self.state == self.STATE_MENU:
                ...
                if self.menu_panel is None:
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if event.button == 1:
                            ...
                            for idx, item_name in enumerate(self.inventory[:self.inventory_max_size]):
                                item_rect = pygame.Rect(500, 130 + idx * 26, 480, 24)
                ...
                if self.selected_tower_obj:
                    # Frissítjük a fejlesztés gomb pozícióját a kiválasztott toronyhoz
                    self.upgrade_button.rect.x = self.selected_tower_obj.x - self.upgrade_button.rect.width // 2
                    self.upgrade_button.rect.y = self.selected_tower_obj.y + 30
    ...
    def draw_hud(self):
        ...
        pygame.draw.rect(self.screen, UI_BG_COLOR, hud_rect)
        pygame.draw.line(self.screen, WHITE, (0, SCREEN_HEIGHT - HUD_HEIGHT), (SCREEN_WIDTH, SCREEN_HEIGHT - HUD_HEIGHT), 3)

        # Gombok rajzolása
        for btn in self.hud_buttons:
            btn.draw(self.screen)

        # Játék statisztikák a panelen (tiszta elrendezés a gombok között)
        # Az X pozíciót a gombok után lévő üres térbe számoljuk
        stats_x = 550
        stats_y_start = SCREEN_HEIGHT - HUD_HEIGHT + 15
        
        money_text = self.font_hud.render(f"Pénz: ${self.money}", True, GOLD)
        gold_text = self.font_hud.render(f"Gold: {self.gold}",True,GOLD)
        crate_text = self.font_hud.render(f"Ládák: {self.crates}",True,WHITE)
        key_text = self.font_hud.render(f"Kulcsok: {self.keys}",True,GOLD)
        lives_text = self.font_hud.render(f"Élet: {self.lives}", True, RED)
        if self.endless_mode:
            wave_str = f"Hullám: {self.wave_manager.current_wave_index + 1}"
        else:
            wave_str = f"Hullám: {self.wave_manager.current_wave_index}/{len(self.wave_manager.waves)}"

        wave_text = self.font_hud.render(wave_str, True, WHITE)
        
        self.screen.blit(gold_text, (stats_x + 220, stats_y_start))
        self.screen.blit(crate_text, (stats_x + 220, stats_y_start + 30))
        self.screen.blit(key_text, (stats_x + 220, stats_y_start + 60))
        self.screen.blit(money_text, (stats_x, stats_y_start))
        self.screen.blit(lives_text, (stats_x, stats_y_start + 30))
        self.screen.blit(wave_text, (stats_x, stats_y_start + 60))
        ...
        if self.is_building and self.selected_tower_type:
            ...
            if pos[1] < SCREEN_HEIGHT - HUD_HEIGHT:
                ...
                # Hatótáv kör
                pygame.draw.circle(self.screen, (255, 255, 255, 100), (center_x, center_y), config["range"], 1)
                ...
                surf.fill(color)
                self.screen.blit(surf, (grid_x, grid_y))

        # Inventory megjelenítése (bal felső sarok, utolsó 5 elem)
        inv_y = 10
    ...
    def draw(self):
        ...
        if self.state == self.STATE_MENU:
            ...
            if self.menu_panel is None:
                ...
                self.screen.blit(sub_title, (SCREEN_WIDTH//2 - sub_title.get_width()//2, 210))
                for btn in self.level_buttons:
                    btn.draw(self.screen)

            elif self.menu_panel == "inventory":
                # ── Inventory panel ──────────────────────────
                sub_title = self.font_medium.render("Inventory", True, GOLD)
                self.screen.blit(sub_title, (SCREEN_WIDTH//2 - sub_title.get_width()//2, 50))

                # ── Bal oldal: Equip slotok + láda nyitás ────
                slot_x = 60
                slot_label = self.font_small.render("Equipped itemek (katt = kivesz):", True, GRAY)
                self.screen.blit(slot_label, (slot_x, 105))

                slot_w, slot_h = 380, 50
                for i in range(4):
                    item_name = self.equipped_items[i]
                    slot_rect = pygame.Rect(slot_x, 130 + i * 58, slot_w, slot_h)
                    # Keret
                    border_color = GOLD if item_name else DARK_GRAY
                    pygame.draw.rect(self.screen, border_color, slot_rect, 2, border_radius=6)
                    if item_name:
                        rarity = ITEMS[item_name][0]
                        ic = RARITY_COLORS[rarity]
                        desc = ITEMS[item_name][4]
                        name_surf = self.font_small.render(f"[{i+1}] {item_name}", True, ic)
                        desc_surf = pygame.font.SysFont("Arial", 16).render(desc, True, GRAY)
                        # Ikon az equip slotban
                        eq_icon = pygame.Surface((36, 36), pygame.SRCALPHA)
                        draw_item_icon(eq_icon, item_name, 0, 0, size=36)
                        self.screen.blit(eq_icon, (slot_x + 8, 132 + i * 58))
                        self.screen.blit(name_surf, (slot_x + 50, 133 + i * 58))
                        self.screen.blit(desc_surf, (slot_x + 50, 152 + i * 58))
                    else:
                        empty_surf = self.font_small.render(f"[{i+1}] — üres slot —", True, DARK_GRAY)
                        self.screen.blit(empty_surf, (slot_x + 8, 147 + i * 58))

                # Aktív bónuszok összesítve
                dmg_b, slow_b, rng_b = self.get_equipped_bonuses()
                bonus_y = 370
                b_label = self.font_small.render("Aktív bónuszok:", True, WHITE)
                self.screen.blit(b_label, (slot_x, bonus_y))
                bonuses = [
                    (f"+{int(dmg_b*100)}% torony sebzés", GREEN),
                    (f"+{int(slow_b*100)}% ellenség lassítás", LIGHT_BLUE),
                    (f"+{int(rng_b*100)}% hatótáv", YELLOW),
                ]
                for bi, (btxt, bcol) in enumerate(bonuses):
                    bs = self.font_small.render(btxt, True, bcol)
                    self.screen.blit(bs, (slot_x, bonus_y + 28 + bi * 26))

                # Láda nyitás szekció
                crate_y = 490
                pygame.draw.line(self.screen, DARK_GRAY, (slot_x, crate_y - 10), (slot_x + slot_w, crate_y - 10), 1)
                gold_s = self.font_small.render(f"Gold: {self.gold}  |  Kulcsok: {self.keys}  |  Ládák: {self.crates}", True, GOLD)
                self.screen.blit(gold_s, (slot_x, crate_y))
                self.inv_open_crate_btn.draw(self.screen)
                self.inv_buy_key_btn.draw(self.screen)
                self.inv_expand_btn.draw(self.screen)

                # ── Jobb oldal: Item lista ────────────────────
                list_x = 500
                inv_count = len(self.inventory)
                list_label = self.font_small.render(
                    f"Itemek ({inv_count}/{self.inventory_max_size}) — bal klikk: equip, jobb klikk: elad",
                    True, GRAY)
                self.screen.blit(list_label, (list_x, 105))
                ...
                if self.inventory:
                    for idx, item_name in enumerate(self.inventory[:self.inventory_max_size]):
                        ...
                        row_text = f"{prefix}{item_name}  [{sell_price}g]"
                        # Mini ikon az item mellett
                        icon_surf = pygame.Surface((22, 22), pygame.SRCALPHA)
                        draw_item_icon(icon_surf, item_name, 0, 0, size=22)
                        self.screen.blit(icon_surf, (list_x, 130 + idx * 26))
                        itxt = self.font_small.render(row_text, True, col)
                        self.screen.blit(itxt, (list_x + 26, 130 + idx * 26))
                else:
                    empty = self.font_small.render("(Még nincs tárgyad — nyiss ládát!)", True, DARK_GRAY)
                    self.screen.blit(empty, (list_x, 150))

                self.inventory_back_button.draw(self.screen)

            elif self.menu_panel == "quit":
                # ── Kilépés megerősítés panel ────────────────
                sub_title = self.font_medium.render("Biztosan kilépsz?", True, RED)
                self.screen.blit(sub_title, (SCREEN_WIDTH//2 - sub_title.get_width()//2, 300))
                for btn in self.quit_panel_buttons:
                    btn.draw(self.screen)
            ...
            if self.selected_tower_obj:
                ...
                pygame.draw.rect(self.screen, YELLOW, (self.selected_tower_obj.grid_x, self.selected_tower_obj.grid_y, GRID_SIZE, GRID_SIZE), 2)
                
                # Fejlesztés információk
                info_text = f"Dmg: {self.selected_tower_obj.damage} -> {self.selected_tower_obj.damage + self.selected_tower_obj.upgrade_damage}"
                
                info_surf = self.font_small.render(info_text, True, WHITE)
                
                # Fekete háttér a szövegnek
                bg_rect = pygame.Rect(self.selected_tower_obj.x - 60, self.selected_tower_obj.y - 60, 120, 25)
                pygame.draw.rect(self.screen, (0, 0, 0, 180), bg_rect)
                pygame.draw.rect(self.screen, WHITE, bg_rect, 1)
                
                self.screen.blit(info_surf, (self.selected_tower_obj.x - 55, self.selected_tower_obj.y - 55))
        ...
        if self.crate_animation is not None:
            self.crate_animation.draw(self.screen)
